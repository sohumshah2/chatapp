...
export const confirmWhetherMatch = (
  receivedMessage,
  ...
  const calculatedHashHex = SHA256(receivedMessage.toString()).toString(
    enc.Hex
  );
  const calculatedHashBigInt = BigInt("0x" + calculatedHashHex);
  const decryptedHash = compute(
    BigInt(receivedEncryptedHash),
    decomposeIntoPowersOf2(e),
    n
  );
